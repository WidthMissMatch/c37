...
architecture structural of pmu_processing_top is
    ...
    signal taylor_dft_active    : std_logic;
    ...
    signal taylor_addr          : std_logic_vector(7 downto 0);
    ...
    signal taylor_freq_out      : std_logic_vector(FREQ_WIDTH-1 downto 0);
    signal taylor_freq_valid    : std_logic;
    signal taylor_rocof_out     : std_logic_vector(FREQ_WIDTH-1 downto 0);
    ...
    signal windowed_sample      : std_logic_vector(SAMPLE_WIDTH-1 downto 0);
    signal windowed_valid       : std_logic;
    signal windowed_index       : std_logic_vector(7 downto 0);
    signal windowed_last        : std_logic;
    signal resamp_last_d1       : std_logic;

    signal freq_filtered        : std_logic_vector(FREQ_WIDTH-1 downto 0);
    signal freq_filtered_valid  : std_logic;
...
begin
    ...
    hann_inst: hann_window
        ...
        port map (
            ...
            sample_out_valid => windowed_valid,
            index_out        => windowed_index,
            coeff_out        => open
        );

    hann_last_delay: process(clk, rst)
    begin
        if rst = '1' then
            resamp_last_d1 <= '0';
            windowed_last  <= '0';
    ...
    dft_buffer_inst: dft_sample_buffer
        ...
        port map (
            clk          => clk,
            rst          => rst,
            write_sample => windowed_sample,
            write_index  => windowed_index,
            write_valid  => windowed_valid,
            write_last   => windowed_last,
    ...
    taylor_dft_inst: taylor_dft_calculator
        ...
        port map (
            ...
            taylor_active => taylor_dft_active,
    ...
    taylor_window_rom_inst: taylor_window_rom
        generic map (
            WINDOW_SIZE => DFT_SIZE,
            COEFF_WIDTH => COEFF_WIDTH
        )
    ...
    cordic_inst: cordic_calculator_256
        ...
        port map (
            ...
            rst           => rst,
            start         => cordic_start,
            real_in       => taylor_c0_real,
            imag_in       => taylor_c0_imag,
    ...
    taylor_freq_est_inst: taylor_frequency_estimator
        generic map (
            INPUT_WIDTH => DFT_OUTPUT_WIDTH,
            FREQ_WIDTH  => FREQ_WIDTH,
            FRAC_BITS   => FRAC_BITS
    ...
    begin
        if rst = '1' then
            ...
            cordic_start <= taylor_result_valid;
        end if;
    end process;
    ...
    freq_damp_inst: freq_damping_filter
        ...
        port map (
            clk            => clk,
            rst            => rst,
            freq_in        => taylor_freq_out,
            freq_valid     => taylor_freq_valid,
            freq_out       => freq_filtered,
            freq_out_valid => freq_filtered_valid,
    ...
    begin
        if rst = '1' then
            ...
            if taylor_freq_valid = '1' and freq_measurement_count < 2 then
            ...
            if freq_measurement_count < 2 then
                ...
                freq_feedback_valid <= '0';
                if freq_filtered_valid = '1' then
                    freq_feedback <= freq_filtered;
                    freq_feedback_valid <= '1';
                end if;
            end if;
        end if;
    ...
    debug_monitor: process(clk)
        ...
        variable taylor_freq_seen : boolean := false;
    ...
    begin
        if rising_edge(clk) then
            ...
            if freq_feedback_valid = '1' and not freq_valid_seen then
                report ">>> [DEBUG] freq_feedback_valid = 1 (initialization frequency active)" severity note;
            ...
            if resamp_cycle_complete = '1' and not cycle_complete_seen then
                report ">>> [DEBUG] First cycle_complete detected" severity note;
                cycle_complete_seen := true;
            end if;

            if dft_buf_full = '1' and not dft_buf_full_seen then
                report ">>> [DEBUG] DFT buffer full (256 samples ready for Taylor-DFT)" severity note;
                dft_buf_full_seen := true;
            end if;

            if taylor_result_valid = '1' and not dft_valid_seen then
                report ">>> [DEBUG] Taylor-DFT result valid (C0/C1/C2 ready)" severity note;
                dft_valid_seen := true;
            end if;

            if cordic_valid = '1' and not phasor_valid_seen then
                report ">>> [DEBUG] First phasor valid (CORDIC complete)" severity note;
                phasor_valid_seen := true;
            end if;

            if taylor_freq_valid = '1' and not taylor_freq_seen then
                report ">>> [DEBUG] First Taylor frequency estimate ready" severity note;
                taylor_freq_seen := true;
            end if;
        end if;
    end process;
    ...
    dft_real_out     <= taylor_c0_real;
    dft_imag_out     <= taylor_c0_imag;
    dft_valid_out    <= taylor_result_valid;

    frequency_out    <= taylor_freq_out;
    freq_valid       <= taylor_freq_valid;

    rocof_out        <= taylor_rocof_out;
    rocof_valid      <= taylor_freq_valid;
