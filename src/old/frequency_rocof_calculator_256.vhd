...
architecture behavioral of frequency_rocof_calculator_256 is
    ...
    constant PI_Q2_13       : signed(THETA_WIDTH-1 downto 0) := to_signed(25736, THETA_WIDTH);
    constant TWO_PI_Q2_13   : signed(THETA_WIDTH-1 downto 0) := to_signed(51472, THETA_WIDTH);
    constant NEG_PI_Q2_13   : signed(THETA_WIDTH-1 downto 0) := to_signed(-25736, THETA_WIDTH);

    constant C_FREQ_SCALE   : signed(31 downto 0) := to_signed(4172344, 32);
    ...
    signal product_freq     : signed(48 downto 0);
...
begin
    ...
    begin
        if rst = '1' then
            ...
            if pipeline_valid(1) = '1' then
                ...
                if delta_theta_raw > resize(PI_Q2_13, THETA_WIDTH+1) then
                    delta_theta <= delta_theta_raw - resize(TWO_PI_Q2_13, THETA_WIDTH+1);
                    large_jump := '1';
                elsif delta_theta_raw < resize(NEG_PI_Q2_13, THETA_WIDTH+1) then
                    delta_theta <= delta_theta_raw + resize(TWO_PI_Q2_13, THETA_WIDTH+1);
                    large_jump := '1';
                else
    ...
    freq_multiply: process(clk, rst)
        variable rounded_product : signed(48 downto 0);
    begin
        if rst = '1' then
            ...
            if pipeline_valid(2) = '1' then
                ...
                rounded_product := product_freq + to_signed(32768, 49);
                delta_f <= resize(shift_right(rounded_product, 16), OUTPUT_WIDTH);
